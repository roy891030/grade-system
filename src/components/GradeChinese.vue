<template>
      ...
      <form @submit.prevent="submitGrade">
        ...
        <select v-model="gradeType" id="gradeType" required>
          <optgroup label="作業">
            ...
            <option value="語詞本">語詞本</option>
            <option value="字音字形本">字音字形本</option>
            <option value="造句本">造句本</option>
            <option value="國語習作">國語習作</option>
          ...
          <tbody>
            <tr v-for="row in 4" :key="'row-' + row">
              <td v-for="col in 10" :key="'col-' + col">
                <input v-model="grades[(row - 1) * 10 + (col - 1)]" type="number" min="0" max="100" required />
              </td>
            </tr>
          </tbody>
        </table>
  ...
  export default {
    ...
    methods: {
      async submitGrade() {
        try {
          const response = await fetch('/api/grades/chinese', {
            method: 'POST',
            headers: {
